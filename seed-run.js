...
async function main() {
  ...
  const expenseCats = [
    { name: "Alquiler", color: "#ef4444" }, { name: "Servicios", color: "#f97316" },
    { name: "Supermercado", color: "#eab308" }, { name: "Transporte", color: "#22c55e" },
    { name: "Salud", color: "#06b6d4" }, { name: "Entretenimiento", color: "#8b5cf6" },
    { name: "Comida", color: "#ec4899" }, { name: "Suscripciones", color: "#6366f1" },
    { name: "Otros Gastos", color: "#6b7280" },
  ];
  const incomeCats = [
    { name: "Sueldo", color: "#16a34a" }, { name: "Freelance", color: "#2563eb" },
    { name: "Inversiones", color: "#7c3aed" }, { name: "Otros Ingresos", color: "#6b7280" },
  ];
  for (const cat of expenseCats) {
    await prisma.category.upsert({
      where: { userId_name: { userId: user.id, name: cat.name } },
      update: {}, create: { ...cat, userId: user.id, type: "EGRESO", icon: "circle" },
    });
  }
  ...
  for (const cat of incomeCats) {
    await prisma.category.upsert({
      where: { userId_name: { userId: user.id, name: cat.name } },
      update: {}, create: { ...cat, userId: user.id, type: "INGRESO", icon: "circle" },
    });
  }
